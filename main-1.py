...
#------------------------------------------>Revisão Gabriel - Inicio
    ...
    teste = sns.barplot(
        x=frequencia_2.
        index,  # `y` recebe o conjunto de valores que correspondem ao eixo x.
        y=frequencia_2,  # `y` recebe o conjunto de valores que correspondem ao eixo y.
        color=
        "blue",  # `color` recebe uma string com o nome de uma cor para as barras.
        label='Qtd.'
    )  # `label` será o "nome" que este gráfico receberá, utilizaremos este "nome" para
...
def get_3_d():
  ...
  if subtopic_graph == 'Selecione uma opção':
    ...
    st.write('salary_in_usd - Histograma 1')
...
def parts_quest_3():
  ...
  graph_project = st.selectbox('Análises', [
      ...
      'company_size', 'salary_in_usd', 'remote_ratio'
  ])
...
\n presença de valores discrepantes e as médias).""")
  ...
  m = pd.concat([media, mediana, minimo, maximo, desv_pad, variancia, q1, q3],
                axis=1,
                keys=columns_r)
  st.dataframe(m)
...
\n dados. Quais informações podemos obter ao utilizar um gráfico de linhas?""")
  ...
  st.write(
      """Como o gráfico de linha visa gerar uma análise de tendência, na base de dados cedida\n
